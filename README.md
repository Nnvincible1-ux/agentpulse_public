...
shows processes attached to a terminal. A terminal whose Mac stopped reporting stays
listed as **Offline** with its last known status for up to 24 hours after its last
activity, so a sleeping or disconnected Mac does not empty the list; closed records,
older offline records and app/background helpers are available under
**History / background**. The count line separates live, offline and waiting terminals. Within each
...
The list previews the captured assistant response. While a terminal is working or
waiting, the preview first shows the current hook activity (for example `Running Bash
· 12 seconds ago` or `Waiting for you · Bash`) and then the last captured response
line. Expanding shows the full response before the reply composer. Process details are a secondary disclosure. The response
