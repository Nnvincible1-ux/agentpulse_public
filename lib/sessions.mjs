  ...
  update(body) {
    ...
    const incoming = body.sessions.map(s => {
      if (!validId(s?.id) || ids.has(s.id) || !["claude", "codex"].includes(s.provider) || !statuses.has(s.status) ||
          ...
          (s.turnStartedAt != null && (!Number.isSafeInteger(s.turnStartedAt) || s.turnStartedAt < 0 || s.turnStartedAt > s.updatedAt)))
        throw fail(400, "Invalid session snapshot.");
      ids.add(s.id);
      return {
    ...
    // Keep live sessions over closed history when the cap is reached; otherwise a flood of closed helpers hides every open terminal.
    this.data.sessions = [...other, ...incoming, ...missing].filter(s => recent(s, now))
      .sort((a, b) => (a.status === "closed") - (b.status === "closed") || b.updatedAt - a.updatedAt).slice(0, 1000);
